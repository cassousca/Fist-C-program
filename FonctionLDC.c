...
liste *ajouter_fin(liste *emp_list, employe *emp){
    ...
    if (emp_list->fin == NULL) /* Cas o� notre liste est vide (pointeur vers fin de liste �  NULL) */
    {
        emp->precedent = NULL; /* On fait pointer p_prev vers NULL */
        emp_list->debut = emp; /* On fait pointer la t�te de liste vers le nouvel �l�ment */
        emp_list->fin = emp; /* On fait pointer la fin de liste vers le nouvel �l�ment */
    }
    else /* Cas o� des �l�ments sont d�j� pr�sents dans notre liste */
    {
        emp_list->fin->suivant = emp; /* On relie le dernier �l�ment de la liste vers notre nouvel �l�ment (d�but du cha�nage) */
        emp->precedent = emp_list->fin; /* On fait pointer p_prev vers le dernier �l�ment de la liste */
        emp_list->fin = emp; /* On fait pointer la fin de liste vers notre nouvel �l�ment (fin du cha�nage: 3 �tapes) */
    }
    emp_list->taille++; /* Incr�mentation de la taille de la liste */
    ecrire_employe(emp);
...
char *get_date(){
    ...
    do
    {
        ...
        int i = c -'0';
        if (isdigit(c) && x < 10){
            if((i < 4 && x == 0) || (p[x-1] - '0' == 3 && x == 1 && i < 2) || (p[x-1] - '0' == 0 && x == 1 && i > 0) || (p[x-1] - '0' < 3 && p[x-1] - '0' != 0 && x == 1)
...
void menu(){
    ...
    printf("|  9\tRecherche d'un employe dans la liste � partir de son code        |\n");
